...
export default function Goals() {
  const { data, updateGoals } = useFitTrack();
  ...
  // Calculate progress
  const currentWeight = [...data.weightEntries].sort(
  ...
  const calculateWeightProgress = () => {
    ...
    const startWeight = [...data.weightEntries].sort(
  ...
  const workoutProgress = data.goals.weeklyWorkouts 
    ? (workoutsThisWeek / data.goals.weeklyWorkouts) * 100 
    : 0;
    ...
    >
            ...
            {data.goals.targetWeight ? (
              <>
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-muted-foreground">→ {data.goals.targetWeight} kg</span>
                </div>
                {weightProgress !== null && (
                  <div className="mt-2">
            ...
            {data.goals.weeklyWorkouts ? (
              <>
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-muted-foreground">/ {data.goals.weeklyWorkouts} this week</span>
                </div>
                <div className="mt-2">
        ...
        <Card>
          ...
          <CardContent>
            {data.goals.dailyCalories ? (
              <div className="text-2xl font-bold">{data.goals.dailyCalories} kcal</div>
            ) : (
              <p className="text-muted-foreground">No goal set</p>
            )}
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Define your fitness targets to track your progress
          </CardDescription>
        </CardHeader>
          ...
          <div className="grid gap-6 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              {latestBodyComp?.bodyFat && (
                <p className="text-xs text-muted-foreground">
                  Current: {latestBodyComp.bodyFat}%
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {latestBodyComp?.muscleMass && (
                <p className="text-xs text-muted-foreground">
                  Current: {latestBodyComp.muscleMass} kg
                </p>
              )}
            </div>
          </div>

          <Button onClick={saveGoals} className="w-full sm:w-auto">
            Save Goals
          </Button>
        </CardContent>
      </Card>
